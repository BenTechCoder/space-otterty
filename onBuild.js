...
const readCssFile = () =>
  readFile('dist/styles.css', (err, data) => {
    if (err) console.log(err)
  })

const readJsFile = () =>
  readFile('dist/index.js', (err, data) => {
    if (err) console.log(err)
  })
...
readCssFile()
  ...
  .then(() => {
    readJsFile()
      ...
      .then(() => {
        fs.rename('dist/styles.css', `dist/styles${hashCss}.css`, (err) => {
          if (err) return console.log(`ERROR: ${err}`)
          console.log(`dist/styles.css > dist/styles${hashCss}.css`)
        })

        fs.rename('dist/index.js', `dist/index${hashJs}.js`, (err) => {
          if (err) return console.log(`ERROR: ${err}`)
